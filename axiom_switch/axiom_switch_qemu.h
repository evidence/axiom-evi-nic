 ...
 * \param raw_eth     packet to send
 *
 * \return send() return value (bytes sent on success, -1 on error)
 */
inline static int
axsw_qemu_send(int sd, axiom_raw_eth_t *raw_eth)
{
    int ret;
    uint32_t axiom_msg_length = htonl(sizeof(*raw_eth));
    ...
    ret = send(sd, raw_eth, sizeof(*raw_eth), 0);
    if (ret != sizeof(*raw_eth)) {
 ...
 *
 * \param sd            socket descriptor connected to QEMU socket backend
 * \param raw_eth     packet received
 *
 * \return recv() return value (bytes received on success, -1 on error)
 */
inline static int
axsw_qemu_recv(int sd, axiom_raw_eth_t *raw_eth)
{
    int ret;
    ...
    ret = recv(sd, raw_eth, axiom_msg_length, MSG_WAITALL);
    if (ret < 0 && (errno == EWOULDBLOCK)) {
        goto skip;
    } else if (ret <= 0) {
        goto err;
